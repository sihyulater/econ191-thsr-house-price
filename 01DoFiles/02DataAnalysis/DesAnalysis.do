...
* closest_station
gen closest_dist = dist_to四城
gen closest_station = "四城"

foreach s in 宜蘭 縣政中心 羅東 {
	replace closest_dist = dist_to`s' if dist_to`s' < closest_dist
	replace closest_station = `"`s'"' if dist_to`s' == closest_dist
}
...
replace zone1 = 1 if not_in_zone & (closest_dist <= 0.5)
replace zone2 = 1 if not_in_zone & (closest_dist <= 1) & (closest_dist > 0.5)
replace zone3 = 1 if not_in_zone & (closest_dist <= 1.5) & (closest_dist > 1)
replace zone4 = 1 if not_in_zone & (closest_dist <= 2) & (closest_dist > 1.5)
replace zone5 = 1 if not_in_zone & (closest_dist <= 2.5) & (closest_dist > 2)
